...
class RPC(object):
    ...
    # NOTE: `server_has_port_security` means the "insecure" flag is not set in /etc/exports.
    #       If it is set, enable this flag and run as root.
            ...
            while True:
                if i > max_bind_tries:
                    if server_has_port_security:
                        raise Exception("Could not find a local port to bind to. "
                                        "(Are you running as root? Are any ports in 500-1024 open?)")
                    else:
                        raise Exception("Could not find a local port to bind to. (Are any ports in 6000-6999 open?)")
                try:
                    if server_has_port_security:
